...
vec3 dirlight::Evaluate(vec3 normal, vec3 view_dir){
    ...
    vec3 col = ambient + diffuse * diff + specular * spec*spec;
    return col;
}
...
vec3 pointlight::Evaluate(vec3 normal, vec3 view_dir, vec3 frag_pos){
    ...
    vec3 col = 
        (ambient + 
        (diffuse * std::max(dot(normal, direction), 0.0f)) + 
        (specular * spec*spec)) * attenuation;
    return col;
}
