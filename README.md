...
The library has only one external dependency, stb_image, which it requires for loading images (for textures). The final output of the library is a FrameBuffer containing all of the pixel colors and depth values for a single frame. You can then use whatever other library you want to output these images to the screen. The demo file, *main.cpp*, uses SDL3.
